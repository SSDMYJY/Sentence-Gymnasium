<template>
  <!-- 登录页容器，限制最大宽度并垂直居中 / Login page container, centered with a max width -->
...
// 获取当前路由 / Obtain the current route
...
// 已登录则跳转到 redirect 或 dashboard / If authenticated, redirect to redirect target or dashboard
...
const onSubmit = async () => {
  ...
  loading.value = true
  try {
    // 调用 store 登录 / Call store login
    await store.login(email.value, password.value, token.value)
    // 登录成功后跳转 / Navigate after success
    await navigateTo((route.query.redirect as string) || localePath('/dashboard'))
  } catch (e: any) {
